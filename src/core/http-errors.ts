...
export class HTTPValidationError extends HTTPError {
    ...
    static fromZodError(err: ZodError): HTTPValidationError {
        ...
        Object.entries(rawErrors).forEach(([key, value]) => {
            if (key === "_errors") { // change key name for _errors key
                formattedErrors["others"] = value;
            } else {
                formattedErrors[key] = (value as unknown as { _errors: string[] })._errors;
            }
        });
...
export const errorHandler = (err: Error, req: Request, res: Response, _: NextFunction) => {
    const body: { success: false, message: string, errors?: SchemaValidationErrors } = { success: false, message: err.message };
